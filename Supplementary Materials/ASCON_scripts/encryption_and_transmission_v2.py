# Write a modified version of the ASCON encryption script that logs the key and nonce for each transmission
...
log_file = 'C:/Users/zahee/OneDrive - Texas State University/Documents/Sertainty project/code_2025/scripts/ascon_transmission_log_detailed.csv'
serial_port = 'COM4'
baud_rate = 9600
interval = 10
VARIANT = "Ascon-128"

# Key for session (should be saved for decryption)
...
def convert_row_to_compact_string(row_dict):
    try:
        ...
        return f"I{row_dict.get('ITEM', '')}F{row_dict.get('FW VERSION', '')}H{row_dict.get('HW VERSIÓN', '')}T{row_dict.get('TEMPERATURE', '')}M{row_dict.get('MILIVOLTS', '')}D{dval}"
    except:
        return None
